...
class PlayerData:
    ...
    def assemble_player_games(self):
        ...
        key_cols = [
            "game_date", "game_num", "day", "winner", "a_score", "b_score", "a_quality", "b_quality",
            "spread", "score_diff", "diff_from_spread", "moneyline", "a_win_prob"
        ]
        ...
        df_long = df_long.with_columns([
            ...
            pl.when(pl.col("team") == "A").then(-pl.col("score_diff")).otherwise(
                pl.col("score_diff")).alias("score_diff"),
            pl.when(pl.col("team") == "A").then(-pl.col("diff_from_spread")).otherwise(
                pl.col("diff_from_spread")).alias("diff_from_spread"),
        ...
        ]).drop(["player_role", "a_teammates", "b_teammates", "a_opponents", "b_opponents"]).join(
        ...
        self.player_games = (df_long.with_columns([
            ...
            ['game_date', 'game_num', 'day', 'player', 'rating', 'team', 'team_score', 'opp_score', 'winner', 'score_diff',
             'win_prob', 'moneyline', 'proj_score_diff', 'diff_from_spread', 'team_quality', 'teammate_quality',
             'opp_quality', 'other_9_players_quality_diff',
             'T1', 'T2', 'T3', 'T4', 'O1', 'O2', 'O3', 'O4', 'O5']
        ).sort(["player", "game_date", "game_num"]).with_columns([

            # player_game_num: Count up within each player, ordered by game_date -> game_num
            pl.col("game_num").cum_count().over("player").alias("player_game_num"),
            ...
            (pl.col("game_num").cum_count().over("player") - pl.col("winner").cum_sum().over("player"))
            .alias("player_loss_num"),
            # playerdaygame_num: Count up within each (player, game_date), ordered by game_num
            pl.col("game_num").cum_count().over(["player", "game_date"]).alias("player_day_game_num"),

            # Identify first_game (1 if first row for player, 0 otherwise)
            (pl.col("game_num") == pl.col("game_num").min().over(["player", "game_date"])).cast(pl.Int8).alias(
                "first_game_of_day"),
            (pl.col("game_num") == pl.col("game_num").max().over(["player", "game_date"])).cast(pl.Int8).alias(
                "last_game_of_day"),
            (pl.col("game_num") == pl.col("game_num").min().over(["game_date"])).cast(pl.Int8).alias("played_first_game"),
            (pl.col("game_num") == pl.col("game_num").max().over(["game_date"])).cast(pl.Int8).alias("played_last_game")
        ]).with_columns([
            # Calculate games_waited (days since last appearance)
            (pl.col("game_num") - pl.col("game_num").shift(1).over(["player", "game_date"]) - 1)
            .fill_null(0)
            .alias("games_waited"),

            # Identify resets: Either a new game_date or a missing game_num
            ((pl.col("game_num") - pl.col("game_num").shift(1).over(["player", "game_date"]) > 1) |
             (pl.col("game_date") != pl.col("game_date").shift(1).over(["player", "game_date"])))
            ...
            pl.when(pl.col("GameReset") == 1).then(pl.lit(0)).otherwise(pl.lit(1)).alias("consecutive_gamesInitial")
        ]).with_columns([
            # Calculate consecutive_games based on resets
            pl.when(pl.col("GameReset") == 1)
            .then(pl.lit(0))
            .otherwise(pl.col("consecutive_gamesInitial").cum_sum().over(["player", "game_date"]))
    ...
    def assemble_player_days(self):
        self.player_days = self.player_games.group_by(["player", "game_date", "day", "rating"]).agg(
            pl.count("player").alias("games_played"),
            pl.sum("winner").alias("wins"),
            (pl.count("player") - pl.sum("winner")).alias("losses"),
            (pl.sum("winner") / pl.count("player")).round(3).alias("win%"),
            pl.mean("win_prob").round(3).alias("exp_win%"),
        ...
        ).sort("game_date", "first_game_of_day", "player", descending=[True, False, False])
    ...
    def combine_player_stats_with_games_groupings(self):
        self.player_data = self.player_data.join(
            self.player_games.group_by(["player"]).agg(
                ...
                (pl.sum('win_prob') / pl.count('player')).round(3).alias('expected_win_pct'),
                ...
                pl.col('teammate_quality').gt(0).mean().round(3).alias('pct_positive_teammates'),
                pl.col('opp_quality').gt(0).mean().round(3).alias('pct_positive_opponents'),
                pl.col('proj_score_diff').gt(0).mean().round(3).alias('pct_games_favorite'),
                (pl.col('opp_quality') < pl.col('teammate_quality')).mean().round(3).alias('pct_games_better_teammates'),
            ...
            self.player_days.group_by(["player"]).agg(
                ...
                (pl.sum('played_first_game') / pl.count('player')).round(3).alias('first_game_of_day_rate'),
                (pl.sum('played_last_game') / pl.count('player')).round(3).alias('last_game_of_day_rate'),
                ((pl.when(pl.col('day') == 'Mon').then(1).otherwise(0)).sum() /
                 self.games.filter(pl.col("day") == "Mon")["game_date"].n_unique()).round(3).alias("mon_rate"),
                ((pl.when(pl.col('day') == 'Wed').then(1).otherwise(0)).sum() /
                 self.games.filter(pl.col("day") == "Wed")["game_date"].n_unique()).round(3).alias("wed_rate"),
                ((pl.when(pl.col('day') == 'Sat').then(1).otherwise(0)).sum() /
                 self.games.filter(pl.col("day") == "Sat")["game_date"].n_unique()).round(3).alias("sat_rate")
    ...
    def calculate_teammate_opponent_pairings(self):
        key_cols = ['game_date', 'game_num', 'day', 'player', 'rating',
                    'team', 'team_score', 'opp_score', 'winner', 'score_diff',
                    'win_prob', 'moneyline', 'proj_score_diff', 'diff_from_spread',
                    'team_quality', 'teammate_quality', 'opp_quality',
                    'other_9_players_quality_diff']


        # Unpivot to transform A1-A5 and B1-B5 into a single "player" column
        self.teammate_games = self.player_games.unpivot(
                index=key_cols,
                on=[f"T{i}" for i in range(1, 5)],
                variable_name="player_role",
                value_name="teammate"
        ).drop('player_role').join(
            self.player_data.select(['player', 'rating']),
            left_on='teammate',
            right_on='player',
            suffix='_teammate'
        ...
        self.opponent_games = self.player_games.unpivot(
                index=key_cols,
        ...
        self.teammates = self.teammate_games.with_columns(
            ...
            .alias("pairing")
        ).group_by(['player', 'teammate', 'pairing']).agg(
        ...
        self.opponents = self.opponent_games.group_by(['player', 'opponent']).agg(
            pl.len().alias("games_played"),  # Count of rows in the group
            pl.n_unique("game_date").alias("days_played"),
            pl.sum("winner").alias("wins"),
            (pl.len() - pl.sum("winner")).alias("losses"),
