...
def parse_args(args=None):
    ...
    parser.add_argument("--include_common_player_tiers", action="store_false")
    parser.add_argument("--min_games_to_not_tier", default=20, type=int)
    parser.add_argument("--run_in_sample", action="store_true")
...
def compute_ratings(args=None) -> Tuple[pl.DataFrame, int]:
    ...
    if args.run_in_sample:
        ratings_df, best_alpha = model_funs_rapm.train_final_model(df=df, players=players)
    else:
