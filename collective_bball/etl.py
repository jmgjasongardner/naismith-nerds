...
def compute_clock(games: pl.DataFrame) -> pl.DataFrame:
    ...
    games = games.join(
        ...
        .agg(pl.col("game_num").max().alias("max_game_num")),
        on="game_date"
    )
    ...
    games = games.join(
        ...
        pl.col("early_short_day").fill_null(False)
    )
    ...
    games = games.with_columns(
        pl.when((pl.col("game_num") == pl.col("max_game_num")) &
    ...
    games = games.drop(["short_game", "max_game_num", "early_short_day", "has_prior_short"])
...
def compute_starting_poss(games: pl.DataFrame) -> pl.DataFrame:
    ...
    prev_games = games.select([
        pl.col("game_date"),
        (pl.col("game_num") + 1).alias("game_num"),  # so prev row lines up with next game
    ...
    games = games.drop([
        "teamA", "teamB", "prev_teamA", "prev_teamB", "prev_winner", "prev_winners", "ret_A", "ret_B"
    ])
