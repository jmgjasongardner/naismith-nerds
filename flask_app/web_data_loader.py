...
def combine_tier_ratings(stats, ratings, tiers) -> pl.DataFrame:
    combined_ratings = stats.join(
        ...
        pl.col("Rating").fill_null(pl.col("Rating_right"))
    ).rename(
        {"player": "Player"}
    ).select(
        ['Player', 'Rating']
    ).sort(
        "Rating", "Player", descending=[True, False]
    )
...
def calculate_game_spreads(games, ratings) -> pl.DataFrame:
    ...
    games_long = games.unpivot(
        ...
        (pl.col("team_role").str.starts_with("A") * pl.col("Rating")).alias("A_Rating"),
        (pl.col("team_role").str.starts_with("B") * pl.col("Rating")).alias("B_Rating")
    ).group_by(["GameDate", "GameNum"]).agg(
    ...
    games_with_spreads = games.join(
        games_long,
        on=["GameDate", "GameNum"],
        how="inner"
    ).with_columns(
        (pl.col("B_SCORE") - pl.col("A_SCORE")).alias("Score_Difference")
    ).with_columns(
        (pl.col("Score_Difference") - pl.col("Spread")).round(3).alias("Difference_From_Spread")
    ).with_columns(
        (pl.col("Spread").abs()).alias("Absolute_Spread"),
        (pl.col("Score_Difference").abs()).alias("Absolute_Score_Difference"),
        (pl.col("Difference_From_Spread").abs()).alias("Absolute_Spread_Difference"),
    )
