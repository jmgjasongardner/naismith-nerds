...
def home():
    return render_template(
        ...
        games=data.games.to_pandas().to_dict(orient="records"),
...
def player_page(player_name):
    ...
    return render_template(
        ...
        player_stats=data.player_data.drop(['rating', 'tiered_rating', 'full_name', 'height', 'position']).filter(pl.col('player') == player_name).to_pandas().to_dict(orient="records"),
        player_rating=data.ratings.filter(pl.col('player') == player_name).to_pandas().to_dict(orient="records"),
        player_games=data.player_games.filter(pl.col('Player') == player_name).drop('Player').to_pandas().to_dict(orient="records"),
