...
df = df.with_columns(pl.when(pl.col('B_SCORE') > pl.col('A_SCORE'))
                             .then(pl.lit('B'))
    ...
    pl.col('Date').dt.strftime('%Y-%m-%d').alias('GameDate')  # Format the date to YYYY-MM-DD
).with_columns(
    pl.col('Date').cum_count().over('GameDate').cast(pl.Int32).alias('GameNum')  # Sequential count per Date
).with_columns(
    (pl.col('GameDate') + '-' + (pl.col('GameNum')).cast(pl.Utf8)).alias('GameId')
).drop('GameDate').filter(pl.col('A_SCORE').is_not_nan())
...
players = df.select(id_cols + team_cols).unpivot(index = id_cols).with_columns(
pl.col('variable').str.extract(r'([AB])', 1).alias('team'),
    ...
    pl.when(pl.col('team') == 'A').then(pl.col('B_SCORE')).otherwise(pl.col('A_SCORE')).alias('opponent_score'),
    (pl.col('team') == pl.col('Winner')).cast(pl.Int8).alias('GameWon')
).with_columns(
    (pl.col('team_score') - pl.col('opponent_score')).alias('point_diff')
)

player_stats = players.group_by('player').agg([
